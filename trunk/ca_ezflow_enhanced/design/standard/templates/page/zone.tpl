...
<div class="block">
    ...
    {def $current_class_identifier = $attribute.object.class_identifier}
    {foreach ezini( 'General', 'AllowedTypes', 'block.ini' ) as $type}
        {if and( ezini( $type, 'AvailableForClasses', 'block.ini' )|contains($current_class_identifier), ezini( $type, 'AvailableForZones', 'block.ini' )|contains(concat($current_class_identifier,'|',$zone.zone_identifier)) )}
